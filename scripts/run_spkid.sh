...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           gmm_train -i 1 -v 5 -T 0.0005 -N 40 -m 40 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
       ...
       gmm_train -i 1 -v 1 -T 0.0005 -N 30 -m 90 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/train_world.gmm $lists/verif/others.train || exit 1
	   ...
	   #   * <code> gmm_verify ... | tee $w/verif_${FEAT}_${name_exp}.log </code>g
       gmm_verify -d $w/$FEAT -e $FEAT -D $w/gmm/$FEAT -E gmm -w train_world $lists/gmm.list $lists/verif/all.test $lists/verif/all.test.candidates | tee $w/verif_${FEAT}_${name_exp}.log || exit 1
       ...
       for filename in $($lists/final/class.test); do
            mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
            EXEC="wav2mfcc 16 spk_8mu/sr_test/$filename.wav $w/$FEAT/$filename.$FEAT"
            echo $EXEC && $EXEC || exit 1
        done
       ...
       gmm_verify -d $w/$FEAT -e $FEAT -D $w/gmm/$FEAT -w train_world -E gmm $lists/gmm.list $lists/final/verif.test $lists/final/verif.test.candidates | tee $w/verification.log
